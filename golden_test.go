...
func loadGoldenFile(t *testing.T, path string) (goldenData, bool) {
	t.Helper()
	...
	// Skip non-golden JSON files (e.g., python_benchmarks.json).
	if gd.Config.MinClusterSize == 0 || gd.Data == nil {
		return gd, false
	}
	return gd, true
}

func goldenMetric(gc goldenConfig) DistanceMetric {
	switch gc.Metric {
	case "manhattan":
...
func goldenConfigToConfig(gc goldenConfig) Config {
	...
	cfg.Metric = goldenMetric(gc)
	// Golden data was generated by the brute-force path; force it so
	// auto-resolution doesn't pick a tree-based algorithm that produces
	// a different (but valid) MST edge ordering.
...
func TestGoldenLabels(t *testing.T) {
	...
	for _, f := range files {
		t.Run(filepath.Base(f), func(t *testing.T) {
			gd, ok := loadGoldenFile(t, f)
			if !ok {
				t.Skip("not a golden test file")
			}
...
// TestGoldenAlgorithms verifies that all algorithm paths produce labels
// equivalent to the brute-force path on the small dataset. The small dataset
// has no equal-weight MST edges, making the dendrogram deterministic regardless
// of MST edge ordering differences between algorithms.
func TestGoldenAlgorithms(t *testing.T) {
	...
	for _, f := range files {
		...
		baseCfg := goldenConfigToConfig(gd.Config)

		// Get brute-force reference labels.
		bruteCfg := baseCfg
		bruteCfg.Algorithm = AlgorithmBrute
		bruteResult, err := Cluster(gd.Data, bruteCfg)
		if err != nil {
			t.Fatalf("%s: brute Cluster() error: %v", filepath.Base(f), err)
		}

		for _, a := range algos {
			t.Run(filepath.Base(f)+"/"+a.name, func(t *testing.T) {
				cfg := baseCfg
				...
				if !labelsEquivalent(bruteResult.Labels, result.Labels) {
					t.Errorf("labels not equivalent to brute-force:\n  brute:  %v\n  %s: %v",
						bruteResult.Labels, a.name, result.Labels)
				}
			})
		}
	}
