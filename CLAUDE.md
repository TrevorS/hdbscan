...
Input data → Cluster(data, cfg)
  → Pairwise distances (parallel)  (parallel.go, distance.go)
  → Core distances (parallel)       (parallel.go, core_distance.go)
  → Mutual reachability (parallel)  (parallel.go, reachability.go)
  → Prim's MST on full matrix      (mst.go)
  → clusterFromMST                 (hdbscan.go — shared post-MST pipeline)
```

The first three stages run with `Config.Workers` goroutines (default: `runtime.NumCPU()`). Prim's MST is inherently sequential.
...
- **Parallel brute-force stages** — `Config.Workers` (default `runtime.NumCPU()`) controls goroutine count for pairwise distances, core distances, and mutual reachability. Row-wise partitioning with no synchronization needed (non-overlapping writes). Falls back to sequential when `Workers == 1`.
...
**Golden tests** (`testdata/`): 28 JSON files generated from the Python reference via `testdata/generate_golden.py`. Covers 5 datasets (small/blobs/moons/blobs10d/blobs50d), 5 metrics (Euclidean/Manhattan/Chebyshev/Minkowski/Cosine), and 4 config variants (EOM/leaf/mcs10/single). Golden tests force `AlgorithmBrute` since the golden data was generated by the brute-force path. Four test groups in `golden_test.go`:
...
**Unit tests**: each pipeline stage has its own `*_test.go`. Edge cases cover n=1, n=2, all-identical points, minClusterSize > n, +Inf distances. Borůvka tests use a `bruteForceBoruvkaTree` mock implementing `BoruvkaTree` for algorithm-level testing independent of real tree implementations. Parallel function tests verify bitwise identity with sequential counterparts across multiple worker counts.

**Regenerating golden data**: `uv run --with hdbscan --with scikit-learn --with numpy python3 testdata/generate_golden.py` — also runs Python benchmarks and writes results to `testdata/python_benchmarks.json`.
